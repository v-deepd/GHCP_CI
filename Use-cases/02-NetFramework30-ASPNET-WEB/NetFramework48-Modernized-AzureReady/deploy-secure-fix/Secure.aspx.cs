...
namespace NetFramework30ASPNETWEB
{
    ...
    public partial class Secure : System.Web.UI.Page
    {
        ...
        /// <summary>
        /// Initialize telemetry if Application Insights is configured
        /// </summary>
        private void InitializeTelemetry()
        {
            ...
            {
                // Application Insights not available - continue without telemetry
                telemetry = null;
            }
        }
        
        /// <summary>
        /// Safe telemetry tracking - only tracks if telemetry is available
        /// </summary>
        private void TrackEvent(string eventName, Dictionary<string, string> properties = null)
        {
            try
            {
        ...
        /// <summary>
        /// Safe exception tracking - only tracks if telemetry is available
        /// </summary>
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (!User.Identity.IsAuthenticated)
                {
                    ...
                    TrackEvent("UnauthorizedAccessAttempt", new Dictionary<string, string> {
                        { "Page", "Secure.aspx" },
                ...
                if (principal != null)
                {
                    ...
                    TrackEvent("SecurePageAccess", new Dictionary<string, string> {
                ...
                else
                {
                    ...
                    TrackEvent("SecurePageError", new Dictionary<string, string> {
        ...
        private void PopulateGroupsList(ClaimsPrincipal principal)
        {
            ...
            try
            {
                ...
                if (itemCount == 0)
                {
                    ...
                    // Log for diagnostics
                    TrackEvent("NoRolesFound", new Dictionary<string, string> {
                        { "UserName", UserNameLabel.Text },
        ...
        private bool CheckAuthorization(ClaimsPrincipal principal)
        {
            try
            {
                // TEMPORARY: Allow all authenticated users until roles are configured
                // TODO: Remove this after setting up app roles in Azure AD
                if (User.Identity.IsAuthenticated)
                {
                    telemetry.TrackEvent("AuthorizationGranted", new Dictionary<string, string> {
                        { "UserName", UserNameLabel.Text },
                        { "Role", "Authenticated (Temporary)" },
                        { "Timestamp", DateTime.UtcNow.ToString("o") }
                    });
                    return true;
                }
